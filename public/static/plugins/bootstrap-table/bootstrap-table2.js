	...
	function () {
	  ...
	  _createClass(BootstrapTable, [{
	    ...
	    value: function initRow(item, i, data, trFragments) {
	      ...
	      var cardViewCols = this.options.cardViewCols;
	      if (cardViewCols < 0)
	      	cardViewCols = 5;

	      var boxwidth = Math.floor(this.$body.width()/cardViewCols) - 10 ;
	      var boxheight = Math.floor(boxwidth*3/4);
	      if (boxheight < 100)
	      	boxheight = 100;
	      ...
	      if (this.options.cardView) {
	      	html.push('<div class="fileview-row file-box" style="width:'+boxwidth+'px; "',  Utils.sprintf(' id="%s"', Array.isArray(item) ? undefined : item._id), " data-index=\"".concat(i, "\""), Utils.sprintf(' data-uniqueid="%s"', Utils.getItemField(item, this.options.uniqueId, false)), Utils.sprintf(' data-has-detail-view="%s"', !this.options.cardView && this.options.detailView && Utils.calculateObjectValue(null, this.options.detailFilter, [i, item]) ? 'true' : undefined), Utils.sprintf('%s', data_), '>');
	        html.push('<div class="fileview-col file">'
	        	+ '<div class="card-views "><span class="corner"></span>');
	      ...
	      this.header.fields.forEach(function (field, j) {
	        ...
	        if (column.checkbox || column.radio) {
	          ...
	          if (_this7.options.cardView ) {
	          	...
	          	text = [_this7.options.cardView ? "<div class=\"card-view file-check".concat(c, "\">") : "<td class=\"bs-checkbox ".concat(c, "\"").concat(class_).concat(style_, ">"), "<label>\n            <input\n            data-index=\"".concat(i, "\"\n            name=\"").concat(_this7.options.selectItemName, "\"\n            type=\"").concat(type, "\"\n            ").concat(Utils.sprintf('value="%s"', item[_this7.options.idField]), "\n            ").concat(Utils.sprintf('checked="%s"', isChecked ? 'checked' : undefined), "\n            ").concat(Utils.sprintf('disabled="%s"', isDisabled ? 'disabled' : undefined), " />\n            <span></span>\n            </label>"), _this7.header.formatters[j] && typeof value === 'string' ? value : '', _this7.options.cardView ? '</div>' : '</td>'].join('');	          
	          	html.push(text);
	          }	          
	          ...
	          if (_this7.options.cardView) {
	            ...
	            if (column.isTitle) {
	            	//_title = '<div class="file-name">'+value+'</div>';
	            	_title = value;
	            } else if (column.isImage) {
	            	//text = '<div class="icon"><a href="#"><i class="fa fa-file"></i></a></div>';	 
	            	_image = '<div class="image"><a href="'+value+'" title="Image from Unsplash" data-gallery=""><img alt="image" class="img-fluid" src="'+value+'"></a></div>';           	
	            } else if (column.isOpt) {
	            	//text = '<div class="icon"><a href="#"><i class="fa fa-file"></i></a></div>';	 
	            	_opt = '<div class="file-opt">'+value+'</div>';           	
	            } else if (column.field == _this7.options.idField) {
		...
		if (_this7.options.cardView) {
			...
			if (_checkbox) {
				_title = '<div class="file-name"><label class="checkbox-inline">'+ _checkbox +  ' '+ _title +'  </label></div>'
			}
			...
			if (_image)
				...
				_image = '<div class="icon"><a href="#"><i class="fa fa-image"></i></a></div>';
				html.push(_image);
			}
	    ...
	    value: function initBody(fixedScroll) {
	      ...
	      if (!this.$body.length) {
	      	this.$body = $('<tbody class="file-manager"></tbody>').appendTo(this.$el);	        
	      ...
	      var nr = 0;
	      var cw = Math.floor(12/this.options.cardViewCols);
	      if (cw < 1)
	      	cw = 1;
	      ...
	      for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
	        ...
	        if (tr && typeof tr === 'string') {
	        	if (this.options.cardView) {
	        		var n = i%this.options.cardViewCols;

	        		if (n === 0) {
	        			tr = '<div class="row"><div class="file-col col-lg-'+cw+' col-md-'+cw+' col-sm-'+cw+'">'+tr+'</div>';
	        			if (i === this.pageTo - 1)
	        				tr += '</div><div class="clearfix"></div>';
	        		} else if (n === this.options.cardViewCols - 1) {
	        			tr = '<div class="file-col col-lg-'+cw+' col-md-'+cw+' col-sm-'+cw+'">'+tr+'</div></div><div class="clearfix"></div>';
	        		} else {
	        			tr = '<div class="file-col col-lg-'+cw+' col-md-'+cw+' col-sm-'+cw+'">'+tr+'</div>';
	        			if (i === this.pageTo - 1)
	        				tr += '</div><div class="clearfix"></div>';
	        		}

        			console.log('n='+n+',tr='+tr);
	        	}
