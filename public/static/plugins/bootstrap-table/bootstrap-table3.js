(function (global, factory) {
	...
	*/
	...
	var DEFAULTS = {
	  ...
	  idField: undefined,
	  selectItemName: 'params[btSelectItem][]',
	  clickToSelect: false,
	  ...
	  iconFormatter: function iconFormatter(value, row, title) {
	  		if (!value || typeof value != 'string')
	  			return false;
			
			return '<div class="image"><a href="'+value+'" '+title+'" data-gallery=""><img alt="image" class="img-fluid" src="'+value+'"></a></div>';
	  },
	...
	function () {
	  ...
	  _createClass(BootstrapTable, [{
	    ...
	    key: "initRowCardView",
	    value: function initRowCardView(item, start, total, data, trFragments) {
	      var _this7 = this;
	      var cols = 6;
	      ...
	      var tr = '<div class="row fileview">';
	      
	      var cw = Math.floor(12/cols);
	      if (cw < 1)
	      	cw = 1;
	      ...
	      for (var i=start; i<end; i++) {
	      	...
	      	if (typeof item.title != "undefined")
	      		_title = _title_value = item.title;
	      	else if (typeof item.name != "undefined")
	      		_title = _title_value = item.name;
			else if (typeof item.description != "undefined")
	      		_title = _title_value = item.description;



	      	this.header.fields.forEach(function (field, j) {

		        var _value = Utils.getItemField(item, field, _this7.options.escape);
      	        var value = '';
      	        var type = '';
		        ...
		        if (column.escape) {
		          _value = Utils.escapeHTML(_value);
		        }
				value = Utils.calculateObjectValue(column, _this7.header.formatters[j], [_value, item, i, field], _value);
				value = typeof value === 'undefined' || value === null ? _this7.options.undefinedText : value;
				...
				if (column.checkbox || column.radio) {
		          ...
		          type = column.radio ? 'radio' : type;
		          var c = column['class'] || '';
		          var isChecked = (value === true || _value || value && value.checked) && value !== false;
		          var isDisabled = !column.checkboxEnabled || value && value.disabled;

	   	          item[_this7.header.stateField] = value === true || !!_value || value && value.checked;

		          _checkbox = [ "<input class='file-check' data-index=\"".concat(i, "\"\n name=\"").concat(_this7.options.selectItemName, "\"\n type=\"").concat(type, "\"\n ").concat(Utils.sprintf('value="%s"', item[_this7.options.idField]), "\n ").concat(Utils.sprintf('checked="%s"', isChecked ? 'checked' : undefined), "\n ").concat(Utils.sprintf('disabled="%s"', isDisabled ? 'disabled' : undefined), " />"), _this7.header.formatters[j] && typeof value === 'string' ? value : '' ].join('');
	            ...
	            if (column.isImage) {
	            	var previewUrl = '';
	            	var previewSmallUrl = value;
	            	var previewLargeUrl = value;

	            	if  (typeof column.previewUrl != "undefined")
	            		previewUrl = item[column.previewUrl];
	            	if  (typeof column.previewSmallFieldName != "undefined")
	            		previewSmallUrl = item[column.previewSmallFieldName];
	            	if  (typeof column.previewLargeFieldName != "undefined")
	            		previewLargeUrl = item[column.previewLargeFieldName];

	            	if (!previewSmallUrl && previewUrl) {
	            		previewSmallUrl = previewUrl;
	            	} 
	            	if (!previewLargeUrl && previewUrl) {
	            		previewLargeUrl = previewUrl;
	            		var pos = previewLargeUrl.indexOf('?');
	            		if (pos > 0)
	            			previewLargeUrl += '&l=1'; //大图
	            	}
	            	
	            	var isVideo = item._type == 1?true:false;
        				var attr = isVideo?"class='videobox' data-id="+item.id:"data-gallery";

	            	//默认格式
	            	_icon = '<div class="image"><a href="'+previewLargeUrl+'" title="'+_title_value+'" '+attr+' ><img alt="image" class="img-fluid" src="'+previewSmallUrl+'"></a></div>';           	
	            } 

	            if (column.isOpt) {
	            	_opt = '<div class="file-opt">'+value+'</div>';           	
	            } 

	            if (column.field == _this7.options.idField && !_title) {
	            	_title = value;
	            	_title_value = _value;
	            }
	            
	            //主建
	            if (column.field == _this7.options.idField) {
	                _id = value;
	            }
	      	...
	      	if (_checkbox) {
				_title = '<div class="file-name"><label class="checkbox-inline" title="'+_title+'">'+ _checkbox +  ' '+ _title +'  </label></div>'
			}
        	...
        	if  (typeof item.previewUrl != "undefined")
        		previewUrl = item.previewUrl;
        	if  (typeof item.previewSmallUrl != "undefined")
        		previewSmallUrl = item.previewSmallUrl;
        	if  (typeof item.previewLargeUrl != "undefined")
        		previewLargeUrl = item.previewLargeUrl;
        	...
        	if (previewSmallUrl && previewLargeUrl) {
							_icon = '<div class="image"><a href="'+previewLargeUrl+'" title="'+_title_value+'" '+attr+' ><img alt="image" class="img-fluid" src="'+previewSmallUrl+'"></a></div>';           	
        	} else {
							_icon = '<div class="icon"><a href="#"><i class="fa fa-image"></i></a></div>';
					}
			}
            
            
			tr += '<div class="file-col col-lg-'+cw+' col-md-'+cw+' col-sm-'+cw+'">';
			tr += ' <div class="file" data-index="'+i+'" data-uniqueid="'+_id+'">';
			tr += '  <span class="corner"></span>'
			//icon
	    ...
	    value: function initBody(fixedScroll) {
	      ...
	      for (var i = this.pageFrom - 1; i < this.pageTo;) {
	        var item = data[i];
	        var tr = '';

	        if (this.options.cardView) {
	        	tr = this.initRowCardView(item, i, this.pageTo, data, trFragments);
	        	i += 6;	        	
	        } else {
	    ...
	    value: function initBodyEvent() {
	      ...
	      this.$body.find('.file').off('dblclick').on('dblclick', function (e) {
	        var $tr = $(this);
	        var rowIndex = $tr.data('index');
	        var item = _this9.data[rowIndex];
	        var index = $tr.data('uniqueid');;
	       ...
	       // console.log('index='+index);
	        ...
	        var value = Utils.getItemField(item, field, _this9.options.escape);
	         ...
	         _this9.trigger('dbl-click-cell', field, value, item, $tr);
	         _this9.trigger('dbl-click-row', item, $tr, field);
	    ...
	    value: function updateSelected() {
	      ...
	      this.$selectItem.each(function (i, el) {
	        if (_self.options.cardView) {
	      		$(el).closest('.file')[$(el).prop('checked') ? 'addClass' : 'removeClass']('selected');	
	      	} else {
