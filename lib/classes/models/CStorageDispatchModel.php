...
class CStorageModel extends CTableModel
{
	...
	protected function formatForView(&$row, &$ioparams=array())
	{
		$status = $row['status'];
	...
	protected function _init_localstorage()
	{
		$cf = get_config();
		
		$webroot = Factory::GetApp()->getWebroot();
	...
	public function getMyStorage()
	{
		...
		$uid = $userinfo['uid'];
		return $this->getStorageInfo(0, $uid);
	}
	
	
	public function getUserStorageInfo($sid, $uid)
	{
		if (!$sid) {
			rlog(RC_LOG_DEBUG, __FILE__, __LINE__, "no sid '$sid'");
			return false;
		}
		
		if (!$uid) {
			rlog(RC_LOG_DEBUG, __FILE__, __LINE__, "no uid '$uid'");
			return false;
		}
		...
		$oid = $res['oid'];				
		$sql = "select * from cms_storage_dispatch where sid=$sid and oid=$oid";
		$sdinfo = $this->_db->get_one($sql);
		if (!$sdinfo) {			
			rlog(RC_LOG_DEBUG, __FILE__, __LINE__, "no storage of oid '$oid'");
			return false;
		}
		
		
		$dispatch = $sdinfo['dispatch'];
		$used = $sdinfo['used'];					
		$free = $dispatch - $used;
